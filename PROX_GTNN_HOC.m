function [X,tnn,trank] = PROX_GTNN_HOC(Y,LAMB)
...
P_O=@(x,LAMB,gamma) 0.*(abs(x)<=LAMB)+((abs(x)-(gamma^2+LAMB^2)*abs(x)./(gamma^2+x.^2)).*sign(x)).*(abs(x)>LAMB);
